function [ u_k3, L1_3, e1_3 ] = GFISTA_full_backtracking( u0, Fu_star, iter2, mu, mu_f, mu_g, lambda, ep, L_0, rho )
disp(['GFISTA_full_backtracking L_0=',num2str(L_0),' ...']);
L(1)=L_0;
tau(1)=1/L(1);
%-------------------------------------------------------
...
F_u3(1)=F_dual( x2,u0,lambda,ep );
%------------------------------------------------------
%%
k=1;
if (((2*D_f(x2,y2,u0))/norm_2_p(x2-y2))<=(rho/tau(k)))
    tau0(k+1)=tau(k)/rho;
else
    tau0(k+1)=tau(k);
end
...
%----------------------------------------------------
F_u3(k+1)=F_dual( x3,u0,lambda,ep );
x1=x2;
x2=x3;
y2=y3;
T=p;
...
%%
u_k3=u0-D_adjoint( x3 ); 
%-----------------------------------------------------
for k=1:(iter2+1)
    e1_3(k)=F_u3(k)-Fu_star;
end
%-----------------------------------------------------
for k=1:iter2+1;
    L1_3(k)=1/tau(k);
